...
interface CalendarProps {
  /** Lista de entradas (logs), incluyendo citas (appointments) */
  logs: LogEntry[];
}

/**
 * Componente de calendario interactivo que permite visualizar citas registradas en un mes y día específicos.
 */
const Calendar: React.FC<CalendarProps> = ({ logs }) => {
  ...
  /**
   * Filtra y ordena las citas del tipo 'APPOINTMENT'.
   */
  ...
  /**
   * Agrupa las citas por fecha (YYYY-MM-DD).
   */
  ...
  // Información del mes actual para construir el calendario
  ...
  /**
   * Devuelve las citas del mes actual.
   */
  ...
  /**
   * Componente visual para representar una cita como "pill".
   */
          ...
          {days.map((day, index) => {
            ...
            return (
              <div
                ...
                className={`relative p-1.5 h-32 bg-white hover:bg-teal-50 cursor-pointer flex flex-col items-start overflow-hidden ${isSelected ? 'ring-2 ring-teal-500 z-10' : ''}`}
              >
                <time
                  dateTime={dateString}
                  className={`font-medium text-sm ${isToday ? 'bg-teal-600 text-white rounded-full h-6 w-6 flex items-center justify-center' : 'text-slate-700'}`}
                >
                  {day.getDate()}
                </time>
                ...
                {hasAppointment && (
                  <div className="mt-1 w-full space-y-1 overflow-y-auto pr-1">
                    {dayAppointments.slice(0, 3).map(appt => (
                      <div
                        key={appt.id}
                        className={`px-1.5 py-0.5 rounded text-xs truncate ${appt.source === 'google' ? 'bg-amber-100 text-amber-800' : 'bg-teal-100 text-teal-800'}`}
                      >
          ...
          <div className="mt-4 space-y-3 max-h-[28rem] overflow-y-auto pr-2">
            {(selectedDate ? appointmentsForSelectedDate : appointmentsForCurrentMonth).length > 0 ? (
              ...
              <p className="text-sm text-slate-500">
                No hay citas para {selectedDate ? 'este día.' : 'este mes.'}
              </p>
            )}
          </div>
        </div>
